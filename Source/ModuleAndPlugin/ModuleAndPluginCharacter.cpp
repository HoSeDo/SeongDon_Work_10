...
void AModuleAndPluginCharacter::BeginPlay()
{
	...
	MyCustomData = NewObject<UTemporaryCharacterData>(this);

	if (MyCustomData)
	{
		float Hp = MyCustomData->MaxHealth;
		float Dmg = MyCustomData->AttackDamage;
		
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 10.0f, FColor::Red, 
				FString::Printf(TEXT("플러그인 데이터 로드 성공! HP: %f, DMG: %f"), Hp, Dmg));
		}
	}
}
